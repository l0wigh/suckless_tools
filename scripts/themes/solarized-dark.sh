...
# Global colors
sed -i '/#define c0/c\#define c0 #073642'  /tmp/Xresources_switcher
...
sed -i '/#define c4/c\#define c4 #268bd2'  /tmp/Xresources_switcher
sed -i '/#define c5/c\#define c5 #d33682'  /tmp/Xresources_switcher
...
sed -i '/#define cf/c\#define cf #fdf6e3'  /tmp/Xresources_switcher
...
# Helix
sed -i '/theme =/c\theme = "solarized-dark"' ~/.config/helix/config.toml

# Zed Editor
sed -i '/"light":/c\"light": "NeoSolarized Dark",' ~/.config/zed/settings.json

# Apply modifications
cp /tmp/Xresources_switcher ~/.Xresources

# Background
feh --bg-fill ~/wallpapers/solarized-dark.jpg
